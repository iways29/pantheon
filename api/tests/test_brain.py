...
from app.brain import EMBEDDING_DIMENSIONS, Brain, BrainError, HashingEmbedder
...
def test_a_stored_fact_comes_back_by_meaning(brain: Brain, tenants: Tenants) -> None:
...
def test_search_ranks_the_relevant_fact_first(brain: Brain, tenants: Tenants) -> None:
    brain.insert_fact(org_id=tenants.org_a, claim="Invoices are paid through Stripe")
    expected = brain.insert_fact(
        org_id=tenants.org_a, claim="The deployment pipeline runs on GitHub Actions"
    )
...
def test_superseded_facts_do_not_surface_as_current(brain: Brain, tenants: Tenants) -> None:
    old = brain.insert_fact(org_id=tenants.org_a, claim="Invoices are paid through Stripe")
    new = brain.insert_fact(org_id=tenants.org_a, claim="Invoices are paid through Xero")
...
def test_superseding_keeps_the_old_claim_readable(brain: Brain, tenants: Tenants) -> None:
...
def test_a_fact_cannot_supersede_itself(brain: Brain, tenants: Tenants) -> None:
    fact = brain.insert_fact(org_id=tenants.org_a, claim="Invoices are paid through Stripe")

    with pytest.raises(ValueError):
        brain.supersede(fact.id, replaced_by=fact.id)


def test_an_empty_claim_is_refused(brain: Brain, tenants: Tenants) -> None:
    with pytest.raises(ValueError):
        brain.insert_fact(org_id=tenants.org_a, claim="   ")


def test_a_claim_with_no_tokens_is_refused(brain: Brain, tenants: Tenants) -> None:
    """Punctuation embeds to a zero vector, which cosine search cannot rank."""
    with pytest.raises(BrainError):
        brain.insert_fact(org_id=tenants.org_a, claim="!!! ???")


def test_search_returns_nothing_for_an_untokenisable_query(brain: Brain, tenants: Tenants) -> None:
    brain.insert_fact(org_id=tenants.org_a, claim="Invoices are paid through Stripe")
